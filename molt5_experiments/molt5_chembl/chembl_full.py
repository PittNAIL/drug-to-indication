    ...
    for task in tasks:
        for model in models:
            ...
            if task == 'indication2smiles':
                ...
                for _, drug in tqdm.tqdm(drugs.iterrows(), total = len(drugs), desc = "Evaluating drugs..."):
            ...
            if task == 'smiles2indication':
                ...
                molt_model = T5ForConditionalGeneration.from_pretrained(model_state).to('cuda')
                tokenizer = T5Tokenizer.from_pretrained(model_state)
                for _, drug in tqdm.tqdm(drugs.iterrows(),total=len(drugs_test), desc = "Evaluating drugs..."):
