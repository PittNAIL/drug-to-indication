...
def eval_drugs(task, model_dir, data, method, tokenizer):
    ...
    if task == "smiles2indication":
        ...
        for drug in tqdm.tqdm(data, total=len(data), desc="Evaluating drugs..."):
        ...
        df_model.to_csv(f"custom_db_{method}_molt5-small_{task}.txt", sep="\t", index=False)
    ...
    for method in methods:
        if method == "finetuned":
            for task in tasks:
                ...
                trainer.train()
                ft_model = f"{output_directory}/checkpoint-18"
