...
def main() -> None:
    ...
    for task in tasks:
            ...
            output_directory = f"custom_molt5_small_db_{task}"
            ...
            if task == 'indication2smiles':
                ...
                model_dir = f"{output_directory}/checkpoint-18"
                ft_model = AutoModelForSeq2SeqLM.from_pretrained(f"{model_dir}").to('cuda')
                model_data = {'description': [], 'ground truth': [], 'output': []}
                for drug in tqdm.tqdm(drugs_test, total = len(drugs_test), desc = "Evaluating drugs..."):
                ...
                df_model.to_csv(f"custom_molt5_small_db_{task}.txt", sep = "\t", index=False)
