...
def eval_drugs(task, model, data, method, tokenizer_ft=None):
    if method == "finetuned":
    ...
    if task == "indication2smiles":
        ...
        for _, drug in tqdm.tqdm(
            data.iterrows(), total=len(data), desc="Generating SMILES from Indications..."
        ):
    ...
    if task == "smiles2indication":
        ...
        for _, drug in tqdm.tqdm(
            data.iterrows(), total=len(data), desc="Generating Indications from SMILES..."
        ):
        ...
        if method == "finetuned":
            for task in tasks:
                for model in models:
                    ...
                    output_directory = f"chembl_ft_{model}_{task}"
                    ...
                    eval_drugs(task, ft_model, drugs_test, method, tokenizer_ft=tokenizer)
